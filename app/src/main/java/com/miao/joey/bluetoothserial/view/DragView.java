package com.miao.joey.bluetoothserial.view;
...
public class DragView extends RelativeLayout implements View.OnClickListener {
        ...
        mDrager = ViewDragHelper.create(this, 5f, new ViewDragHelper.Callback() {
            ...
            public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
                ...
                if (changedView == fgView)
                    getParent().requestDisallowInterceptTouchEvent(fgView.getLeft() != 0);
                if (mDragStateListener != null) {
                    if (left == 0) {
                        mDragStateListener.onCloseed(DragView.this);
    ...
    protected void onFinishInflate() {
        ...
        if (getChildCount() != 2)
            throw new IllegalArgumentException("must contain only 2 child view");
        fgView = getChildAt(1);
        bgView = getChildAt(0);
        if (!(fgView instanceof ViewGroup && bgView instanceof ViewGroup))
            throw new IllegalArgumentException("Foreground and Background must be a subClass of GroupView");
    ...
    public interface DragStateListener {
        ...
        void onCloseed(DragView dragView);
